...
// Shows a compact A4-ish portrait preview above the mouse.
...
function ensurePreview() {
  let container = document.getElementById(PREVIEW_ID) as HTMLDivElement | null;
...
function showPreview(url: string, x: number, y: number) {
  const container = ensurePreview();
  const img = container.querySelector<HTMLImageElement>('#pdf-preview-image');
  if (img && img.src !== url) {
    img.src = url;
  }
  positionPreview(x, y);
...
function setupPdfPreview() {
  const triggers = document.querySelectorAll<HTMLElement>('[data-pdf-preview-url]');
...
if (typeof window !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', setupPdfPreview);
  } else {
    setupPdfPreview();
  }
}
